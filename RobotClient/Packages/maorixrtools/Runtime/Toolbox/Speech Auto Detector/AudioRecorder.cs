...
{
    //Handler signature
    public delegate void EndSpeakFoundHandler(float [] data, int frequency, int channels, int samples);
    //Event signature
    public event EndSpeakFoundHandler EndSpeakEvent;
    ...
    public void StopRecording()
    {
        ...
        if (samplesCount > 0)
        {
            ...
            if (EndSpeakEvent != null)
            {
                ...
                EndSpeakEvent(samples, _frequency, _sourceRecording.clip.channels, samplesCount);
            }
       
           // _Test.RecognizeVoice(clip);
